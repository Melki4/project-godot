...
const VLAD_BOSS = preload("uid://e1oksxqt0j26")
...
func on_waves_finished():
	...
	vlad_boss.init()
	vlad_boss.vlad_damaged.connect(ui.change_boss_hb_value)
	vlad_boss.vlad_died.connect(ui.on_vlad_died)
	ui.init_boss_health_bar(vlad_boss.get_health())

func on_player_damaged(curent_health: int):
	ui.decrease_health(curent_health)
