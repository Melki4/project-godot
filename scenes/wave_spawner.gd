...
signal starting_wave(wave_index, total_waves)
...
func on_spawn_timeout() -> void:
	...
	match enemy_to_spawn:
		...
		EnemyType.SKELETON:
			...
			if !current_wave.skeleton_count:
				enemy_types.erase(EnemyType.SKELETON)
	if !current_wave.skeleton_count && !current_wave.mummy_count && !current_wave.devils_count:
